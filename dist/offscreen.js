...
${u}`, c = n.createShaderModule({ code: d, label: e.name });
  ...
  var MODEL_URL = chrome.runtime.getURL("wasm/nior_classifier.onnx");
  var WASM_PREFIX = chrome.runtime.getURL("wasm/");
  var LABELS = [
    "main_content",
    "navigation",
    "call_to_action",
  ...
  var LOW_CONF_THRESHOLD = 0.65;
  var ortSession = null;
  async function initSession() {
    ye.wasm.wasmPaths = WASM_PREFIX;
    ye.wasm.simd = true;
    ye.wasm.numThreads = Math.min(navigator.hardwareConcurrency ?? 2, 4);
    const modelBuffer = await fetch(MODEL_URL).then((r) => r.arrayBuffer());
    ortSession = await vf.create(modelBuffer, {
  ...
  async function runInference(batch) {
    ...
    const tensor = new Ke("float32", flat, [n, 72]);
  ...
  var GEMINI_PROMPT_TMPL = (json) => `Given the following DOM element properties, classify the element into exactly one of [main_content, navigation, call_to_action, form, heading, sidebar, media, advertisement]. Respond only with a JSON object {"category": string, "confidence": number}.
